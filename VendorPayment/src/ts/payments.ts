...
function updateBalancesUI(): void {
    ...
    if (acc1El) acc1El.textContent = accounts[0].balance.toLocaleString("en-US", { style: "currency", currency: "USD" });
    if (acc2El) acc2El.textContent = accounts[1].balance.toLocaleString("en-US", { style: "currency", currency: "USD" });
...
function isPaydayForVendor(index: number): boolean {
    ...
    if (index < 5) return true; // Vendors 1–5 weekly
    if (index >= 5 && index < 10) {
...
// ------------------ Report Generation ------------------
function generateCurrentReport(): void {
    if (typeof Excel === "undefined") {
    ...
    Excel.run(async (context) => {
        ...
        const completedRows = vendors
            .filter(v => v.status && v.status.startsWith("Paid") || v.status?.startsWith("On-demand"))
...
document.addEventListener("DOMContentLoaded", () => {
    ...
    if (processBtn) processBtn.addEventListener("click", processScheduledPayments);

    const onDemandBtn = document.getElementById("triggerOnDemandBtn");
    if (onDemandBtn) onDemandBtn.addEventListener("click", triggerOnDemandPayment);
